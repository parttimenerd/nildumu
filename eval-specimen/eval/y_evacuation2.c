...
short is_solution(short berths[20][2], short N) {
	short sum = 0;
	short i = 0;
	while (i < 20) {
		sum = sum + ((berths[i])[0]);
...
short AtLeast(short Capacity[20], short z, short b) { return Capacity[z] >= b; }

short abs(short val) {
    return val > 0 ? val : -val;
}

short Nearby(short Loc[20][2], short z, short l[2], short d) {
	return abs((Loc[z][0]) - (l[0])) + abs((Loc[z][0]) - (l[0])) <= d;
}

extern short Capacity[20];
extern short Loc[20][2];
...
int main() {
    ...
    berths[16][0], berths[16][1],
    berths[17][0], berths[17][1],
    berths[18][0], berths[18][1],
    berths[19][0], berths[19][1]};
