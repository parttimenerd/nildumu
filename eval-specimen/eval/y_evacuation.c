...
short is_solution(short berths[10][2], short N) {
	short sum = 0;
	short i = 0;
	while (i < 10) {
		sum = sum + ((berths[i])[0]);
...
short AtLeast(short Capacity[10], short z, short b) { return Capacity[z] >= b; }

short abs(short val) {
    return val > 0 ? val : -val;
}

short Nearby(short Loc[10][2], short z, short l[2], short d) {
	return abs((Loc[z][0]) - (l[0])) + abs((Loc[z][0]) - (l[0])) <= d;
}

extern short Capacity[10];
extern short Loc[10][2];
...
int main() {
    ...
    short result[20] = {berths[0][0], berths[0][1],
