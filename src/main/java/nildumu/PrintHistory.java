...
public class PrintHistory {
    ...
    static class HistoryEntry {
        ...
        static HistoryEntry create(Map<String, Lattices.AppendOnlyValue> map, Optional<HistoryEntry> prev, Function<Lattices.Value, Set<Lattices.Bit>> reachabilityCalculator){
        ...
        static HistoryEntry create(Map<String, Lattices.AppendOnlyValue> map, Optional<HistoryEntry> prev, HistoryEntry base){
            return new HistoryEntry(prev, map.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey,
    ...
    static class HistoryPerGlobalEntry {
        ...
        HistoryPerGlobalEntry(Optional<HistoryPerGlobalEntry> prev, String name, Lattices.AppendOnlyValue value, long reachableBitNum, Set<Lattices.Bit> reachableBitsForDiff) {
            this.prev = prev;
            this.name = name;
            this.value = value;
        ...
        HistoryPerGlobalEntry(Optional<HistoryPerGlobalEntry> prev, String name, Lattices.AppendOnlyValue value, Function<Lattices.Value, Set<Lattices.Bit>> reachabilityCalculator) {
            this.prev = prev;
            this.name = name;
            this.value = value;
            this.difference = prev.map(h -> value.difference(h.value)).orElse(value);
        ...
        ReduceResult<Lattices.AppendOnlyValue> reduceAppendOnly(){
            HistoryPerGlobalEntry currentHist = this;
            ReduceResult<Lattices.AppendOnlyValue> current = new ReduceResult<>(currentHist.value.clone(), false);
